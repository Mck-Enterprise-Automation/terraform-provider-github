...
func testSweepDeleteRepositoryFiles(meta interface{}, branch string) error {
	...
	for _, f := range files {
		if strings.HasPrefix(*f.Name, "tf-acc-") {
			log.Printf("Deleting repository file: %s, repo: %s/test-repo, branch: %s", *f.Name, owner, branch)
			opts := &github.RepositoryContentFileOptions{Branch: github.String(branch)}
			if _, _, err := client.Repositories.DeleteFile(context.TODO(), owner, "test-repo", *f.Name, opts); err != nil {
				return err
			}
		}
	}
